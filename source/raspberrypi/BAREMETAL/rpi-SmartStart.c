...
#include "windows.h"			// PACKED/ALIGN/PACKEDALIGN
...
{--------------------------------------------------------------------------*/
struct PACKEDALIGN(4) GPIORegisters {
	...
	uint32_t reserved12[12];										// 0xA0-0xCC  reserved
	uint32_t GPPINMUXSD; 											// 0xD0  GPPINMUXSD 
	uint32_t reserved13[4];											// 0xD4-0xE0  reserved
	uint32_t GPPUPPDN[4]; 											// 0xE4-0xF0  GPPUPPDN 
};

/*--------------------------------------------------------------------------}
{  RASPBERRY PI SYSTEM TIMER HARDWARE REGISTERS - BCM2835 Manual Section 12	}
{--------------------------------------------------------------------------*/
struct PACKEDALIGN(4) SystemTimerRegisters {
...
{--------------------------------------------------------------------------*/
struct PACKEDALIGN(4) ArmTimerRegisters {
	uint32_t Load;													// 0x00
...
{--------------------------------------------------------------------------*/
struct PACKEDALIGN(4) IrqControlRegisters {
...
;{-------------------------------------------------------------------------*/
struct PACKEDALIGN(4) MailBoxRegisters {
...
{--------------------------------------------------------------------------*/
struct PACKEDALIGN(4) MiniUARTRegisters {
	mu_io_reg_t IO;													// +0x0
	mu_ie_reg_t IER;												// +0x4
	mu_ii_reg_t IIR;												// +0x8
...
{--------------------------------------------------------------------------*/
struct PACKEDALIGN(4) PL011UARTRegisters {
...
{--------------------------------------------------------------------------*/
struct PACKEDALIGN(4) QA7Registers {
...
static_assert(sizeof(struct GPIORegisters) == 0xF4, "Structure GPIORegisters should be 0xF4 bytes in size");
...
****************************************************************************/
#define GPIO ((volatile ALIGN(4) struct GPIORegisters*)(uintptr_t)(RPi_IO_Base_Addr + 0x200000))
#define SYSTEMTIMER ((volatile ALIGN(4) struct SystemTimerRegisters*)(uintptr_t)(RPi_IO_Base_Addr + 0x3000))
#define IRQ ((volatile ALIGN(4) struct IrqControlRegisters*)(uintptr_t)(RPi_IO_Base_Addr + 0xB200))
#define ARMTIMER ((volatile ALIGN(4) struct ArmTimerRegisters*)(uintptr_t)(RPi_IO_Base_Addr + 0xB400))
#define MAILBOX ((volatile ALIGN(4) struct MailBoxRegisters*)(uintptr_t)(RPi_IO_Base_Addr + 0xB880))
...
#define QA7 ((volatile ALIGN(4) struct QA7Registers*)(uintptr_t)(0x40000024))
...
static const char* ARM8_A53_STR = "cortex-a53";
static const char* ARM8_A72_STR = "cortex-a72";
...
const char* RPi_CpuIdString (void)
{
	...
	case 0xd03:														// ARM 8 CPU(Cortex A53)
		return ARM8_A53_STR;										// Return the ARM string
	case 0xd08:														// ARM 8 CPU(Cortex A72)
		return ARM8_A72_STR;										// Return the ARM string
	default:
...
bool gpio_fixResistor (unsigned int gpio, GPIO_FIX_RESISTOR resistor) 
{
	if ((gpio < MAX_GPIO_NUM) && (resistor <= 2))					// Check GPIO pin number and resistor mode valid
	{
		...
		if (RPi_CpuId.PartNumber == 0xd08) {
			...
			uint32_t shift = (gpio % 16) << 1;
			uint32_t bits = GPIO->GPPUPPDN[gpio / 16];
			bits &= ~(3 << shift);
			bits |= (pull << shift);
			GPIO->GPPUPPDN[gpio / 16] = bits;
			return true;
		}
...
/*-[gpio_setGppinmuxsd]----------------------------------------------------}
. Set the GPPINMUXSD for RPI4.
.-------------------------------------------------------------------------*/
void gpio_setGppinmuxsd(uint32_t mode) {
	GPIO->GPPINMUXSD = mode;
}
...
{
	uint32_t ALIGN(16) message[data_count+3];
...
/*==========================================================================}
{				     PUBLIC ARM CPU SPEED GET ROUTINES						}
{==========================================================================*/
/*-[ARM_getcorespeed]--------------------------------------------------------}
. RETURN: cpu speed
.--------------------------------------------------------------------------*/
uint32_t ARM_getcpuspeed()
{
	uint32_t Buffer[5];

	if (mailbox_tag_message(&Buffer[0], 5, MAILBOX_TAG_GET_CLOCK_RATE, 8, 8, CLK_ARM_ID, 0)) {
		return Buffer[4];
	}
...
. RETURN: core speed
.--------------------------------------------------------------------------*/
uint32_t ARM_getcorespeed()
{
	uint32_t Buffer[5];

	if (mailbox_tag_message(&Buffer[0], 5, MAILBOX_TAG_GET_CLOCK_RATE, 8, 8, CLK_CORE_ID, 0)) {
		return Buffer[4];
	}
...
. RETURN: emmc speed
.--------------------------------------------------------------------------*/
uint32_t ARM_getemmcspeed()
{
	// EMMC clock id
	int clockid;
	if (RPi_IO_Base_Addr == 0xfe000000) {
		clockid = CLK_EMMC2_ID;
	} else {
		clockid = CLK_EMMC_ID;
	}
	...
	if (mailbox_tag_message(&Buffer[0], 5, MAILBOX_TAG_GET_CLOCK_RATE, 8, 8, clockid, 0)) {
		return Buffer[4];
	}
...
bool ARM_setmaxspeed (int(*prn_handler) (const char *fmt, ...)) {
	...
	if (prn_handler) prn_handler("EMMC frequency %u Hz\n", ARM_getemmcspeed());
	...
	if (mailbox_tag_message(&Buffer[0], 5, MAILBOX_TAG_GET_MAX_CLOCK_RATE, 8, 8, CLK_ARM_ID, 0)) {
		if (mailbox_tag_message(&Buffer[0], 5, MAILBOX_TAG_SET_CLOCK_RATE, 8, 8, CLK_ARM_ID, Buffer[4])) {
			...
			if (mailbox_tag_message(&Buffer[0], 5, MAILBOX_TAG_GET_MAX_CLOCK_RATE, 8, 8, CLK_CORE_ID, 0)) {
				if (mailbox_tag_message(&Buffer[0], 5, MAILBOX_TAG_SET_CLOCK_RATE, 8, 8, CLK_CORE_ID, Buffer[4])) {
					if (prn_handler) prn_handler("CORE frequency set to %u Hz\n", Buffer[4]);
					return true;
				}
			}
		}
